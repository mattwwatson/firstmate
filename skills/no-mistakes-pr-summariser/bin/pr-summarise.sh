...
# The SKILL.md beside this script holds that measurement and the forge behaviour
# behind every decision here.
#
# Usage: pr-summarise.sh <pr-url> --opening-file <path> [--dry-run] [--keep-dir <dir>]
#
...
#                 supplied by the caller: the author's own words when a file of
#                 them exists, and otherwise one derived from the published body
#                 and the diff. SKILL.md owns that judgement.
...
# text (pr-identity.sh owns that string), in the whole footer line this script
...
# body write can report success without changing anything, which has been
# observed for `gh pr edit --body`.
#
# WHAT IT NEEDS. GitHub goes entirely through the gh CLI, so an authenticated
# `gh` on PATH is the whole requirement. Bitbucket Cloud has no such CLI, so it
# needs an Atlassian account email and an Atlassian API token with the
# pullrequest:write scope, read from BITBUCKET_EMAIL and BITBUCKET_API_TOKEN by
# the pr-forge-env.sh sibling. With either missing that sibling refuses and
# names the one that is absent, rather than attempting a half-authenticated
# request; nothing is written on that path.
#
# WHERE THE ORIGINAL GOES. Under PR_SUMMARISER_STATE_DIR when set, and otherwise
...
# the first at its own keychain-backed credential, and the tests stub both.
FORGE_CREDENTIAL_BIN="${PR_SUMMARISER_FORGE_BIN:-$SCRIPT_DIR/pr-forge-env.sh}"
GH_BIN="${PR_SUMMARISER_GH_BIN:-gh}"

# shellcheck source=skills/no-mistakes-pr-summariser/bin/pr-identity.sh
. "$SCRIPT_DIR/pr-identity.sh"
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/pr-summarise.XXXXXX") || exit "$EX_FORGE_FAILED"
