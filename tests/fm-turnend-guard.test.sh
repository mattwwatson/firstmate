...
# The Stop registration itself is asserted by
# test_settings_stop_hook_uses_project_dir below; this case owns only the
# all-tools PreToolUse stamp, the captain-wait deferral's input.
test_settings_registers_turn_stamp() {
  local settings stamp
  ...
  pass ".claude/settings.json: registers the turn stamp on all tools"
}

# The retired --notify-wake hand-off pass must stay retired: a stale adapter
# registration would otherwise wedge every Stop on a usage error
# (docs/turnend-guard.md "Superseded: the one-shot hand-off pass").
test_settings_has_no_retired_notify_wake() {
  local settings
  settings="$ROOT/.claude/settings.json"
  [ -f "$settings" ] || fail "tracked .claude/settings.json is missing"
  if grep -q 'notify-wake' "$settings"; then
    fail ".claude/settings.json still registers the retired --notify-wake flag"
  fi
  pass ".claude/settings.json: carries no retired --notify-wake registration"
}
...
test_tracked_claude_entries_inert_under_grok() {
  ...
  # Six, not upstream's five: this fork also tracks bin/fm-turn-pretool-stamp.sh,
  # the Claude-only turn-activity stamp the captain-wait deferral releases on
  # (docs/watcher-continuity.md "Captain-wait deferral").
