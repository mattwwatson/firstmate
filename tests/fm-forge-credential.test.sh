...
test_bootstrap_reports_a_broken_credential_at_session_start() {
  ...
  git -C "$home/projects/hexbattle" remote add origin 'git@bitbucket.org:mattw_watson/hexbattle.git'
  git init -q "$home/projects/gh-only"
  git -C "$home/projects/gh-only" remote add origin 'https://github.com/owner/repo.git'
  fakebin="$dir/bin"
