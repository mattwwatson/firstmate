...
`bin/fm-arm-command-policy.mjs` is also the sole owner of firstmate's shell classification: it exports the tokenizer and command-position analysis, which the sibling cd-guard seatbelt (`bin/fm-cd-pretool-check.sh`, `docs/cd-guard.md`) and the crew kill-guard (`bin/fm-kill-pretool-check.sh`, `docs/kill-guard.md`) reuse instead of duplicating shell lexing.
