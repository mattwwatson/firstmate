...
- A shell run as a DIRECT stage of a pgrep-fed pipeline with a literal `-c` payload executing any kill verb (same captain decision - the shell stage inherits the pipe's stdin): `pgrep -f dev | bash -c "xargs kill"` is denied.
...
- A direct shell stage under the same conditions: PID-only with no pgrep-fed pipe (`echo x | bash -c "kill 123"`) or fed by a worktree-scoped pgrep (`pgrep -f '<worktree>' | bash -c 'xargs kill'`).
