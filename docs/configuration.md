...
When a running home advances and its loaded instruction surface (`AGENTS.md`, `bin/`, `.agents/skills/`, or `skills/no-mistakes-pr-summariser/bin/`, the list `changed_instr` owns in `bin/fm-ff-lib.sh`) changed, bootstrap sends the re-read nudge itself through the stable `fm-<id>` selector and reports the exact completed send as `BOOTSTRAP_INFO:`.
...
**Its write capability is the captain's provisioning choice, checked against its real scopes.** The recommended scopes are repository, pull request, and pipeline READ, under which an unattended reader cannot push, merge, or otherwise change a repository, and the write actions stay dormant because the forge itself refuses every write.
...
The resolver has three write actions, each gated by the same pull-request write scope: the pull-request merge POST driven by `bin/fm-bb-pr-merge.sh` (docs/bitbucket-merge-watch.md "Stage 4"); the pull-request comment POST, which `bin/fm-pr-comment.sh` uses to post a ship task's Manual-testing section and `bin/fm-pr-reshape.sh` to post the detail a reshaped description moves out; and the pull-request description PUT driven by `bin/fm-pr-reshape.sh`, which replaces the description of the pull request it names (docs/pr-description-reshape.md).
...
FM_FLEET_PAUSE_TIMEOUT=300         # operator-facing: seconds /pause keeps re-verifying before it names the workers that did not confirm; raise it for a slow-turning fleet, since a worker mid-turn has to finish its step first. It bounds the report only and is never a reason to report safe-to-close early
