...
A declared pause firstmate itself commanded, through the captain-invoked fleet pause, is the one the always-on watcher absorbs completely rather than on that cadence: while the pause record is open and the task's own current status confirms that exact pause instance, the confirmation signal, the single live-agent sighting, and the later recheck are all absorbed, because a fleet where every task is correctly paused would otherwise burn the supervision cycle nobody is left to re-arm ([docs/fleet-pause.md](fleet-pause.md) owns why).
A task that never confirmed keeps its ordinary supervision, and any later status event retires the confirmation and restores it at once.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` and then dispatches by provider: GitHub through `gh-axi pr merge <n> --repo <owner>/<repo>` exactly as before, Bitbucket through `bin/fm-bb-pr-merge.sh`.
The helper requires a full canonical PR URL, defaults to squash on both forges, preserves explicit merge-method flags (mapping gh-style names to Bitbucket strategies), and rejects malformed URLs or repo override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
A Bitbucket merge (see [docs/bitbucket-merge-watch.md](bitbucket-merge-watch.md) "Stage 4") refuses a red, pending, or unreadable build verdict before any request exists, handles Bitbucket's synchronous, asynchronous, and rate-limited merge answers explicitly, and reports success only on a pull-request state read back from the API as MERGED; whether the shared credential can merge at all is the captain's provisioning choice, detected from its real scopes and enforced by the forge.
...
Durable project-intrinsic agent knowledge lives in each project's committed `AGENTS.md`, with `CLAUDE.md` as a symlink or a pointer-only `@AGENTS.md` include; `bin/fm-ensure-agents-md.sh`'s header owns the exact accepted pointer shape.
