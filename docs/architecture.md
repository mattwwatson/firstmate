...
A reader that stays broken is still surfaced rather than silently absorbed: every `FM_WEDGE_UNREADABLE_SURFACE_COUNT` consecutive unreadable probes wake the first mate with a reason that names the failing reader and asserts neither a wedge nor a stop, and any readable verdict drops that count.
