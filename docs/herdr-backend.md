...
- Lease acquisition atomically publishes home-and-task-bound recovery metadata with the exact Herdr endpoint before the historical binding, and final task metadata atomically replaces that recovery record only after later spawn setup completes.
