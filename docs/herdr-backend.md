...
- Treehouse return uses durable started and completed states bound to authoritative `treehouse status` snapshots, so an interrupted return is retried only while ownership is unchanged and a confirmed return is never repeated against a reassigned worktree.
