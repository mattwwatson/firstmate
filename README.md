...
- **Optional secondmates** - opt in to persistent second mates that run from isolated firstmate homes with their own `FM_HOME`, state, projects, and session lock, supervising project clones or a project-less firstmate-repo domain, either locally or as a whole home on an SSH-reachable host, with guarded updates, session-start liveness checks, and recovery that never turns an unavailable remote route into a local replacement.
