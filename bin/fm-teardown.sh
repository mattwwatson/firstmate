...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      ...
      "$(fm_quiesce_probe_file "$sub_state" "$child_id")"
  done
}
...
if [ "$KIND" != secondmate ]; then
  ...
  # Two aborts, covering different halves of one property: cleanup must not
  # remove a worker while a run it owns is still going, and must refuse rather
  # than proceed when it cannot confirm the run stopped.
  #
  # conclude_task_no_mistakes_run above is the CONFIRMATION half. It reaches only
  # a run parked at a gate, but it re-reads that run's state after aborting and
  # refuses the teardown unless the read proves the run stopped.
  #
  # This is the BREADTH half, and runs second on purpose: whatever is still going
  # by here is a run the narrow abort does not reach. A run does not end when its
  # pull request turns green - it stays in its monitoring phase holding the
  # shared validator until the request lands or the run is aborted.
  # bin/fm-nm-abort.sh reaches any run on the task's branch with no terminal
  # outcome, monitoring included, and is the single owner of that abort (the
  # fleet pause uses it too). Seven runs orphaned this way once jammed the shared
  # validator, which then refused to restart and blocked every other lane.
  # Neither call subsumes the other: the first proves a parked run stopped, the
  # second reaches runs the first never inspects.
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$(fm_quiesce_probe_file "$STATE" "$ID")"
