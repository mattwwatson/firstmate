...
if network_phase; then
  ...
  # A forge credential firstmate holds itself is probed over the network exactly
  # like the gh probe above, so it belongs in this stage rather than either local
  # block. Both report only; the read-only guard lives in forge_news_already_reported.
  __fm_timing_stamp=$(fm_timing_now_ms)
  forge_credential_check
  merge_capability_check
