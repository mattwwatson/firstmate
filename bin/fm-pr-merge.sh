...
# The canonical PR URL is parsed by bin/fm-pr-lib.sh and dispatched by provider.
# A GitHub pull request is addressed through gh-axi by the derived owner and
# repository, exactly as it always has. A Bitbucket pull request merges through
# bin/fm-bb-pr-merge.sh, which owns the build gate, the 200/202/409/429/555
# merge protocol, and the confirmed-MERGED success rule; whether the Bitbucket
# credential CAN merge is a captain provisioning choice enforced by the forge
# itself (docs/configuration.md "Forge credentials"). A GitLab merge request is
# addressed through glab by the project URL rebuilt from the parsed host and
# path, so any instance works and no host is hardcoded.
#
