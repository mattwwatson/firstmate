...
#
# The stored identity is provider-tagged, and the file sourced below owns what
# each of its parts means. Every consumer here re-derives that identity from the
...
# The provider-tagged identity variables, the URL parser, the reshape marker
# and its private directory, and the one owner of per-forge comment routing all
# live in the portable summariser library rather than here. The installable
# summariser skill needs every one of them and can reach nothing outside its own
# directory, so they sit inside it and this file sources them - one definition,
# two callers, rather than a copy that drifts.
# Resolved with `cd -P`, so the sibling is found relative to this file's PHYSICAL
# location. Test fixtures build synthetic roots by symlinking the real bin/ into
# them, and a logical resolution would look for skills/ beside the synthetic root
# rather than beside the real one, which fails with a bare "No such file".
# shellcheck source=skills/no-mistakes-pr-summariser/bin/pr-identity.sh
. "$(cd -P "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/skills/no-mistakes-pr-summariser/bin/pr-identity.sh"
