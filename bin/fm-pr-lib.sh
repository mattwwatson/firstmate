...
# bin/fm-pr-merge.sh addresses GitHub by owner/repository. A bitbucket URL is
# dispatched to bin/fm-bb-pr-merge.sh, and a gitlab URL leaves owner/repository
# empty and is addressed by FM_PR_HOST and FM_PR_PATH instead, so a merge
# request on any instance resolves without a hardcoded host.
fm_pr_url_parse() {
