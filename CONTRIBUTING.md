...
Before scoping a firstmate bug, proposing such a change, or making one, load the agent-only `firstmate-coding-guidelines` skill (`.agents/skills/firstmate-coding-guidelines/SKILL.md`).
...
bin/fm-lint.sh   # lint the canonical shell surface it owns; the single owner CI and the no-mistakes gate both run
