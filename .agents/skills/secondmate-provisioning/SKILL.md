...
Inheritance also copies the literal default-off `config/herdr-child-workspaces` feature flag, so the primary's explicit `on` opt-in gives every ordinary crew in the supervisor tree its own Herdr workspace while absence converges the whole tree back to the default tab-per-task layout.
