...
description: >-
  ...
  Use before deciding any ask-user finding, regardless of the project's autonomy grants, to distinguish corrections within accepted intent from product or engineering contract expansion that requires the captain.
...
1. Check the project's configured authority first, through `bin/fm-project-mode.sh`.
   A project grants `findings`, `merge`, `merge-unobservable`, and `local-merge` independently, and holding one never implies another; bare `+yolo` is shorthand for `findings,merge,local-merge`.
   The grant that governs answering an ask-user finding is `findings` and nothing else, so a project registered `+yolo:merge` has not authorized it.
   Without the `findings` grant, every ask-user finding belongs to the captain, and the remaining steps structure that escalation rather than authorize an autonomous answer.
...
4. Keep the decision within standing `findings` authority when the Fix is genuinely necessary to satisfy the accepted contract, even when the correction is technically difficult or requires complex architecture that the captain explicitly requested.
...
9. Escalate when the grants are unclear.
   The captain owns every gate not granted, so an unrecognised or ambiguous registration resolves to the least permission, never to the wider reading.
...
- Fixing a concrete defect that violates an original acceptance criterion stays within `findings` authority, regardless of implementation difficulty.
...
- A project registered `+yolo:merge` still routes every ask-user finding to the captain: a merge grant says nothing about answering findings.
