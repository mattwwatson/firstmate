...
The declared inherited config set, owned by `secondmate-provisioning`, also includes `config/herdr-child-workspaces` alongside `config/crew-dispatch.json`, `config/crew-harness`, and `config/backlog-backend`.
