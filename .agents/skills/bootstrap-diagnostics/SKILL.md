...
- `FORGE_CREDENTIAL: <forge>: <reason>` - this home tracks a repository on a forge whose credential firstmate holds itself, and that credential cannot be used, so merge detection and build results for every repository on that forge are unavailable until it is fixed.
  ...
  "merge is granted on <projects> but the credential cannot merge" is the capability mismatch line: those projects hold the merge grant while the credential's real scopes lack pull-request write, so reading and merge detection keep working but autonomous merges on them will be refused.
  It reports every session start while the mismatch persists, and the captain resolves it by either re-scoping the credential to include pull-request write or removing the merge grant from those projects; a read-only credential with no merge grants anywhere is a healthy shape and never produces it.
...
- `NUDGE_SECONDMATES: secondmate <id>: send failed: <reason>` - the secondmate sweep fast-forwarded a running secondmate home and its loaded instruction surface (the watched paths `changed_instr` owns in `bin/fm-ff-lib.sh`) changed, but the deterministic `fm-send.sh fm-<id>` re-read nudge failed.
