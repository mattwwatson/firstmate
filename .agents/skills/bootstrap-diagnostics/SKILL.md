...
- `FORGE_CREDENTIAL: <forge>: <reason>` - this home tracks a repository on a forge whose credential firstmate holds itself, and that credential cannot be used, so merge detection and build results for every repository on that forge are unavailable until it is fixed.
  ...
  A repository the probe could not see never produces a line at all: the forge authenticates before it resolves the resource, so that answer proves the credential was accepted and only names a repository-visibility fact, which is why one probe settles the credential for the whole forge.
  ...
  Report it once, plainly, and move on; bootstrap says it once per home and then stays silent, so do not treat a later silence as the problem having been fixed.
  A lock-refused session reports that one without recording it, so the same line arriving again in the session that holds the lock is the expected handover, not a regression.
